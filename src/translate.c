...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    instruction = ((((((instruction + rs) << 5) + rt) << 5)  + rd) << 6) + funct;
...
int write_shift(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    instruction = ((((((((instruction + rs) << 5) + rt) << 5)  + rd) << 5) + shamt) << 6) + funct;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_jr(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = ((instruction + rs) << 21) + funct;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_addiu(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    imm = args[2];
    // opcode rs rt imm
    uint32_t instruction = (instuction + opcode) << 5;
    instruction = (instruction + rs) << 5;
    instruction = (instruction + rt) << 16;
    instuction = (instruction + imm);
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_ori(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    imm = args[2];
    // opcode rs rt imm
    uint32_t instruction = (instruction + opcode) << 5;
    instruction = (instruction + rs) << 5;
    instruction = (instruction + rt) << 16;
    instruction += imm;
...
int write_lui(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    imm = args[1];
    uint32_t instruction = (instrucion + opcode) << 10;
    instruction = (instruction + rt) << 16;
    instruction += imm;
    write_inst_hex(output, instruction);
    return 0;
...
int write_branch(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
    ...
    int32_t offset = (label_addr - (addr + 4)) / 4; // take out plus 4 if buggy
    uint32_t instruction = (instruction + opcode) << 5;
    instruction = (instruction + rs) << 5;
    instruction = (instruction + rt) << 16;
    instruction += offset;
